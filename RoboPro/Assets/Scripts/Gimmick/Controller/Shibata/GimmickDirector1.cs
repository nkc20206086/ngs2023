            ...
            for (BlockID id = BlockID.Command_Red; id <= BlockID.Command_Black; id++)
            {
                if (dic.ContainsKey(id))
                {
                    ...
                    if (dic.ContainsKey(id + 100))
                    {
                        ...
                        StageObject notInitalizedGimmick = notInitalizedGimmick = dic[id + 100].Find(x => { if (x == null) return false; return !x.GetComponent<GimmickController>().IsInitalized; })?.GetComponent<StageObject>();

                        while (notInitalizedGimmick != null)
                        {
                            GameObject gimmick = new GameObject();
                            gimmicks.Add(gimmick);
                            List<GameObject> combinedGimmicks = new List<GameObject>();
                            GimmickController c = gimmick.AddComponent<GimmickController>();
                            Vector3 parentPos = new Vector3();

                            void CheckCombine(Vector3Int pos)
                            {
                                for (int i = 0; i < dic[id + 100].Count; i++)
                                {
                                    if (dic[id + 100][i].GetComponent<GimmickController>().GetComponent<StageObject>().Position == pos)
                                    {
                                        GimmickController c = dic[id + 100][i].GetComponent<GimmickController>();
                                        if (!c.IsInitalized)
                                        {
                                            Debug.Log("OK");
                                            parentPos += dic[id + 100][i].transform.position;
                                            c.Initalize();
                                            combinedGimmicks.Add(dic[id + 100][i]);
                                            CheckCombineAround(pos);
                                        }
                                        return;
                                    }
                                }
                            ...
                            void CheckCombineAround(Vector3Int pos)
                            {
                                CheckCombine(pos);
                                CheckCombine(pos + new Vector3Int(1, 0, 0));
                                CheckCombine(pos + new Vector3Int(-1, 0, 0));
                                CheckCombine(pos + new Vector3Int(0, 1, 0));
                                CheckCombine(pos + new Vector3Int(0, -1, 0));
                                CheckCombine(pos + new Vector3Int(0, 0, 1));
                                CheckCombine(pos + new Vector3Int(0, 0, -1));
                            }

                            CheckCombineAround(notInitalizedGimmick.Position);

                            parentPos /= combinedGimmicks.Count;
                            gimmick.transform.position = parentPos;

                            for (int i = 0; i < combinedGimmicks.Count; i++)
                            {
                                combinedGimmicks[i].transform.SetParent(gimmick.transform);
                            }
                            notInitalizedGimmick = dic[id + 100]?.Find(x => { if (x == null) return false; return !x.GetComponent<GimmickController>().IsInitalized; })?.GetComponent<StageObject>();
                        }
                        accessPoints[accessPoints.Count - 1].GimmickSubscrive(gimmicks);
                    }
                }
            }
