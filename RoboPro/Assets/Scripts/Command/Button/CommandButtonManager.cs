...
namespace Command
{
    ...
    public class CommandButtonManager : MonoBehaviour
    {
        ...
        public void Intialize(Action<int> sw, Action<int> ma,Action<int> st)
        {
            ...
            {
                int index = i;                                                              // インデックスをローカルで保存
                strageCommandButtons[i].onClick.AddListener(() => st(index));               // onClickを登録(ストレージインデックスを受け取るアクション)
            }
        ...
        public void MainButtonTextRewriting(MainCommand[] commands)
        {
            for (int i = 0; i < mainCommandButtons.Length; i++) // コマンドの数だけ実行
            {
                ...
                else
                {
                    ...
                    useMainCommandButtons[i, 2].GetComponentInChildren<TextMeshProUGUI>().text = default;
                }
            }
        }
        ...
        public void StrageButtonTextRewriting(CommandBase[] commands)
        {
            ...
            { 
                ...
                strageCommandButtons[i].GetComponentInChildren<TextMeshProUGUI>().text = commands[i] != null ? commands[i].GetString() : default;
            }
        }
    }
}