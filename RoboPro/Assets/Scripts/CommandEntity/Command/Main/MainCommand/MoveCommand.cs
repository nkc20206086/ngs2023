...
namespace Command.Entity
{
    ...
    public class MoveCommand : MainCommand
    {
        ...
        public MoveCommand(bool lock_m, bool lock_n, bool lock_c,
        ...
        public override void CommandExecute(CommandState state, Transform targetTransform)
        {
            ...
            if (Vector3.Distance(basePos, targetTransform.position) > Mathf.Abs(num_local))     // 原点からの移動距離が設定数値を超えているなら
            {
                targetTransform.position = basePos + (GetVec() * Mathf.Abs(num_local)) * mag;   // 対象の位置を対象の座標に変更
                completeAction?.Invoke();                                                       // コマンド完了時処理を実行
            }
            else                                                                                // まだ移動距離が設定数値を超えていないなら
            {
                targetTransform.position += GetVec() * mag;                                     // 座標値を倍率を反映した数値分移動する
            }
        }
    }
}
