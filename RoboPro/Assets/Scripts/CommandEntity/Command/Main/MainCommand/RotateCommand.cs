...
namespace Command.Entity
{
    ...
    public class RotateCommand : MainCommand
    {
        ...
        public override void CommandExecute(CommandState state, Transform targetTransform)
        {
            ...
            if (Mathf.Abs(Quaternion.Angle(baseQuat, targetTransform.rotation)) > Mathf.Abs(num_local))     // 指定された角度分回転していれば
            {
                targetTransform.eulerAngles = baseAngle + (GetVec() * Mathf.Abs(num_local) * mag) ;         // eulerAngleを初期回転値に回転を反映した値にする
                completeAction?.Invoke();                                                                   // コマンド完了時アクションを実行する
            }
            else
            {
                targetTransform.Rotate(GetVec() * mag);                                                     // 方向に倍率を反映した値回転する
            }
        }
    }
}